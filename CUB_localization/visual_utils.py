...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        # print('test_unseen_loc:', len(matcontent['train_unseen_loc'].squeeze()))
        # print('test_unseen_small_loc:', len(matcontent['test_unseen_small_loc'].squeeze()))
        ...
        if opt.ol:
            # print('self.trainval_loc:', len(self.trainval_loc))
            self.trainval_loc = np.append(self.trainval_loc, self.train_unseen_loc)
            # print('self.trainval_loc2:', len(self.trainval_loc))
            self.test_unseen_loc = self.test_unseen_small_loc
            # self.trainval_loc = matcontent['trainval_loc'].squeeze() - 1
...
def default_flist_reader(opt, image_files, img_loc, image_labels, dataset):
    ...
    for image_file, image_label in zip(image_files, image_labels):
        if dataset == 'CUB':
            image_file = opt.image_root + image_file[0].split("images/")[1]
        elif dataset == 'AWA1':
...
def compute_per_class_acc(test_label, predicted_label, nclass):
    ...
    # print("test_label", np.unique(test_label))
    # print("predicted_label", np.unique(predicted_label))

    # print("nclass:", nclass)
    predicted_label = np.array(predicted_label)
    acc_per_class = []
    # print("len", len(test_label), len(predicted_label))
    acc = np.sum(test_label == predicted_label) / len(test_label)
    # print('overall acc:', acc)
    for i in range(len(nclass)):
        # print("test_label:", test_label, i)
        idx = (test_label == i)
...
def add_attri_selection_paths(opt, img_path, correct, predicted_label, attri_name, allclasses_name, attri_id=None, attri_rank=None, attri_weight = None, dataset = None):
    ...
    mask_path = os.path.join(opt.image_root, 'visual/{}/{}/masks/{}/'.format(opt.vis_type, opt.train_id, attri_name[attri_id].strip()))
