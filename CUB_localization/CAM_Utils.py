...
def show_cam_on_image_plt(img, mask, file_path="cam.jpg", raw_path=None):
    ...
    if raw_path is not None:
        raw = np.float32(img)
        raw = raw / np.max(raw)
        cv2.imwrite(raw_path, np.uint8(255 * raw))  # raw
...
def save_mask_as_pdf(mask, img, img_dim, fig):
    mask = cv2.resize(mask, (img_dim, img_dim))
    # print('\nmask:\n', mask)
    mask_max = np.max(mask)
    mask_min = np.min(mask)
...
def generate_mask_BB_max_area_center(mask, bird_BB, scale=4, out_of_edge=False):
    """
    in this function, the center of the bounding box is the center of a max area
    the size of the bounding box is tuned in order to control the ratio to be smaller than 2
    :return   a dict {'x1', 'x2', 'y1', 'y2'}
    """
    bird_w = bird_BB[2] - bird_BB[0]
    bird_h = bird_BB[3] - bird_BB[1]
    ...
    for x in range(0, 224 - sub_area_s, inter):
        for y in range(0, 224 - sub_area_s, inter):
            BB_sum = np.sum(mask[x:x + sub_area_s, y:y + sub_area_s])
            if BB_sum > best_BB_sum:
                best_BB_sum = BB_sum
                best_x1 = x
                best_y1 = y
    best_center_x = best_x1 + int(sub_area_s/2)
    best_center_y = best_y1 + int(sub_area_s/2)
    x1 = best_center_x - int(mask_w/2)
    x2 = best_center_x + int(mask_w/2)
    y1 = best_center_y - int(mask_h / 2)
    y2 = best_center_y + int(mask_h / 2)
    if not out_of_edge:
        if x1 < 0:
            x1 = 0
        if x2 > 223:
    ...
    return {'x1': x1, 'x2': x2, 'y1': y1, 'y2': y2}, (best_center_x, best_center_y)
...
def get_IoU_Image(idx, imgs, maps, save_dir, save_att_idx, groups, attri_names, KPs, bird_BB, scale , out_of_edge, max_area_center):
    ...
    for group_name, group_dims in groups.items():
        ...
        if group_name != 'others':
            img_IoU[group_name] = []
            part_id = BB_parts.index(group_name)
            ...
            for group_dim in group_dims:
                # time_1 = time.time()
                mask = maps[idx, group_dim, :, :]
                mask = cv2.resize(mask, (224, 224))
                if max_area_center:
                    mask_BB_dict, (mask_c_x, mask_c_y) = generate_mask_BB_max_area_center(mask, bird_BB, scale, out_of_edge)
                else:
                    mask_BB_dict, (mask_c_x, mask_c_y) = generate_mask_BB(mask, bird_BB, scale)
                ...
                for KP_idx in KP_idxs:
                    ...
                    if KPs[KP_idx][2] !=0:
                        KP_x = KPs[KP_idx][0]
                        KP_y = KPs[KP_idx][1]
                        KP_M_dis = np.sqrt(np.square(KP_x - mask_c_x) + np.square(KP_y - mask_c_y))
                        if KP_M_dis < KP_M_dis_best:
                            KP_M_dis_best = KP_M_dis
                            KP_best_x = KP_x
                            KP_best_y = KP_y
                        # print('KP_M_dis_best:', KP_M_dis_best)
                if KP_M_dis_best == 1000:
                    img_IoU[group_name].append(-1)
                else:
                    bird_w = bird_BB[2] - bird_BB[0]
                    bird_h = bird_BB[3] - bird_BB[1]
                    mask_w = int(bird_w / scale)
                    ...
                    KP_BB_dict = {'x1': KP_x1, 'x2': KP_x2, 'y1': KP_y1, 'y2':KP_y2}
                    ...
                    if save_dir and save_att_idx[idx] == 1:
                        ...
                        img_dim = img.shape[0]
                        save_mask_as_pdf(mask, img, img_dim, fig)
                        plt.savefig(os.path.join(group_dir, "{}_attri_{:0>3}_{}.png".format(save_dir.split('/')[-1], group_dim, attri_names[group_dim])), dpi=100)
                        ...
                        img_pil.save(os.path.join(group_dir, "original_image.png"))


                        draw = ImageDraw.Draw(img_pil)

                        draw.rectangle(((KP_BB_dict['y1'], KP_BB_dict['x1']), (KP_BB_dict['y2'], KP_BB_dict['x2'])), fill=None,
                                       outline='red', width=2)

                        # for KP_id, KP_center in enumerate(KP_centers):
                        #     if KP_center[0] != -1:
                        #         draw.rectangle(((KP_center[1], KP_center[0]), (KP_center[1] + 5, KP_center[0] + 5)),
                        #                        fill=BB_color[KP_id])

                        draw.rectangle(
                            ((mask_BB_dict['y1'], mask_BB_dict['x1']), (mask_BB_dict['y2'], mask_BB_dict['x2'])),
                            fill=None, outline='blue', width=2)

                        img_pil.save(os.path.join(group_dir, "{}_attri_{:0>3}_{}_BB_IoU_{:.3f}.png".format(save_dir.split('/')[-1], group_dim, attri_names[group_dim], IoU)))
                        print('image saved to', os.path.join(group_dir, "{}_attri_{:0>3}_{}_BB_Io_{:.3f}.png".format(save_dir.split('/')[-1], group_dim, attri_names[group_dim], IoU)))
    # print("img_IoU:", img_IoU)
    return img_IoU

def calculate_atten_IoU(input, impath, save_att_idx, maps, vis_groups, KP_root=None, save_att=False, scale=4, out_of_edge=False, max_area_center=False):
    """
    :param input: input image
    :param impath: image paths
    :param maps: maps with size (batch_size, 312, 7, 7)
    ...
    :param scale: width(height) of bounding box / width(height) of bird.
    :param max_area_center: if max_area_center, the center of the bounding box is the maximum value in attention map
    if max_area_center == False: the sum of attention map value in bounding box to is highest
    :return:
    """
    img_raw_show = tensor_imshow(input)  # (64, 3, 224, 224)
    attri_names = refine_attri_names('./save_files/attri_name.txt')
    batch_IoU = []
    for i, path in enumerate(impath):
        ...
        img_IoU = get_IoU_Image(i, img_raw_show, maps, save_dir, save_att_idx, vis_group, attri_names, KPs, bird_BB, scale, out_of_edge, max_area_center)
        time_3 = time.time()
...
def show_cam_on_image(img, mask, file_path="cam.jpg", raw_path=None):
    ...
    cam = cam / np.max(cam)
    cv2.imwrite(file_path, np.uint8(255 * cam))  # grad-cam
...
def show_image_pixels(img, mask, file_path="cam.jpg", raw_path=None, perc=0.3):
    ...
    cv2.imwrite(file_path, np.uint8(255 * start))  # grad-cam
...
class CAM:
    ...
    def __call__(self, x, attribute, kernel_size=None):
        ...
        for i in range(attri_dim):
            ...
            cam_features.append(cv2.resize(mask, (self.img_dim, self.img_dim)))  
        return cam_features
